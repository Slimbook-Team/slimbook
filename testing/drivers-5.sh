...
if [ $(command -v "apt") ]; then # Ubuntu
    if [ "$(grep Name="SlimbookOS" /etc/os-release)" ]; then # && [ "$(grep 24 /etc/os-release)" ]; then
        ...
        desktop apt
        if [ ! "$(grep completion ~/.bashrc)" ]; then
        echo "# SLIMBOOK_INICIO
    ...
    model "EVO" "apt install slimbook-yt6801-dkms -y"
    model "Prox|Executive|Titan|Hero|EVO|CREA" "apt install linux-headers-amd64 slimbook-qc71-dkms -y"
    grub "EVO|CREA" "update-grub"
    ...
    grub "EVO|CREA" "grub-mkconfig -o /boot/grub/grub.cfg"
    ...
    model "EVO" "dnf install slimbook-yt6801-kmod -y"
    model "Prox|Executive|Titan|Hero|EVO|CREA" "dnf install slimbook-qc71-kmod -y"
    gpu " otro " "dnf install akmod-nvidia"
    grub "EVO|CREA" "grub2-mkconfig -o /boot/grub2/grub.cfg"
